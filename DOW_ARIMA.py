...
dow_ser = get_data("DOW","2020-01-01","2023-07-01")  #training data
#google_ser_oot = get_data("GOOG","2023-01-01","2023-09-26")
dow_ser_oot = get_data("DOW","2023-07-01","2023-09-26")  #test data
dow_ser_tot = get_data("DOW","2020-01-01","2023-09-26")  #all data

print(dow_ser)

#  Check if time series is stationary with ADF test(it is not)
result = adfuller(dow_ser)
print(result)
...
nrdiffs = ndiffs(dow_ser.values, test="adf")
...
#  Function to create manual ARIMA model with p,d,q (0,2,1 here)
model = ARIMA(dow_ser.values, order=(0,2,1))
result = model.fit()
...
auto_arima = pm.auto_arima(dow_ser.values, stepwise=False, seasonal=False)
...
forecast_test = result.forecast(len(dow_ser_oot))
forecast_test_auto = auto_arima.predict(len(dow_ser_oot))
...
ax1.plot(dow_ser_oot.values)
ax1.set_title("Manual forecast (b) vs auto forecast (o) vs oot data (g)")
ax2.plot(dow_ser_tot)
ax2.set_title("full data")

mae = mean_absolute_error(dow_ser_oot.values, forecast_test)
mape = mean_absolute_percentage_error(dow_ser_oot.values, forecast_test)
rmse = mean_squared_error(dow_ser_oot.values, forecast_test)

print(f'mae - manual: {mae}')
print(f'mape - manual: {mape}')
print(f'rmse - manual: {rmse}')

mae_auto = mean_absolute_error(dow_ser_oot.values, forecast_test_auto)
mape_auto = mean_absolute_percentage_error(dow_ser_oot.values, forecast_test_auto)
rmse_auto = mean_squared_error(dow_ser_oot.values, forecast_test_auto)
