...
ge_ser = get_data("GE","2020-01-01","2023-07-01")  #training data
#google_ser_oot = get_data("GOOG","2023-01-01","2023-09-26")
ge_ser_oot = get_data("GE","2023-07-01","2023-09-26")  #test data
ge_ser_tot = get_data("GE","2020-01-01","2023-09-26")  #all data


#  Check if time series is stationary with ADF test(it is not)
result = adfuller(ge_ser)
print(result)
...
nrdiffs = ndiffs(ge_ser.values, test="adf")
...
#  Function to create manual ARIMA model with p,d,q (7,1,7 here)
model = ARIMA(ge_ser.values, order=(7,1,7))
result = model.fit()
...
auto_arima = pm.auto_arima(ge_ser.values, stepwise=False, seasonal=False)
...
forecast_test = result.forecast(len(ge_ser_oot))
forecast_test_auto = auto_arima.predict(len(ge_ser_oot))
...
ax1.plot(ge_ser_oot.values)
ax1.set_title("Manual forecast (b) vs auto forecast (o) vs oot data (g)")
ax2.plot(ge_ser_tot)
ax2.set_title("full data")

mae = mean_absolute_error(ge_ser_oot.values, forecast_test)
mape = mean_absolute_percentage_error(ge_ser_oot.values, forecast_test)
rmse = mean_squared_error(ge_ser_oot.values, forecast_test)

print(f'mae - manual: {mae}')
print(f'mape - manual: {mape}')
print(f'rmse - manual: {rmse}')

mae_auto = mean_absolute_error(ge_ser_oot.values, forecast_test_auto)
mape_auto = mean_absolute_percentage_error(ge_ser_oot.values, forecast_test_auto)
rmse_auto = mean_squared_error(ge_ser_oot.values, forecast_test_auto)
