...
gme_ser_tot_arr = gme_ser_tot.values
nrdata_tot = len(gme_ser_tot)

gme_ser_oot_arr = gme_ser_oot.values
nrdata_oot = len(gme_ser_oot)

gme_ser_arr = gme_ser.values
nrdata_train = len(gme_ser)

#Scale the data and make sure there is no information leak
scaler = MinMaxScaler(feature_range=(0,1))
scaled_data_tot = scaler.fit_transform(gme_ser_tot_arr.reshape(-1,1))

scaler = MinMaxScaler(feature_range=(0,1))
scaled_data_test = scaler.fit_transform(gme_ser_oot_arr.reshape(-1,1))

scaler = MinMaxScaler(feature_range=(0,1))
scaled_data_training = scaler.fit_transform(gme_ser_arr.reshape(-1,1))
...
x_test = []
y_test = gme_ser_tot_arr.reshape(-1,1)[nrdata_train:, :]
